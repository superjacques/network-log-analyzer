...
class NetworkLogAnalyzerApp:
    ...
    def _build_macos_classic_ui(self):
        ...
        Some managed/older macOS Python/Tk combinations create a valid window
        but fail to paint ttk widgets. The earlier compatibility UI proved
        classic Tk widgets render reliably on those machines, so macOS keeps
        the complete application layout without depending on ttk.
        """
        self.root.configure(background="white")

        # --- Top control bar ---
        ctrl_frame = tk.Frame(self.root, background="white", padx=8, pady=8)
        ctrl_frame.pack(fill=tk.X)

        tk.Label(
            ctrl_frame, text="Hours back:", background="white", foreground="black"
        ).pack(side=tk.LEFT)

        self.hours_var = tk.StringVar(value="24")
        tk.Spinbox(
            ctrl_frame, from_=1, to=168, width=5, textvariable=self.hours_var
        ).pack(side=tk.LEFT, padx=(4, 10))

        self.scan_btn = tk.Button(
            ctrl_frame, text="Scan Logs", command=self._start_scan
        )
        self.scan_btn.pack(side=tk.LEFT, padx=4)

        self.diag_btn = tk.Button(
            ctrl_frame, text="Run Diagnostics", command=self._start_diagnostics
        )
        self.diag_btn.pack(side=tk.LEFT, padx=4)

        self.export_btn = tk.Button(
            ctrl_frame, text="Export Report", command=self._export_report
        )
        ...
        # Timeline and events use plain text on macOS so no ttk widget is
        # required anywhere in the rendering path.
